...
const Sidebar: React.FC = () => {
  ...
  const savedMissions = useQuery((api as any).missions?.get) || [];
  const saveMissionAction = useMutation((api as any).missions?.save || (() => {}));
  const removeMissionAction = useMutation((api as any).missions?.remove || (() => {}));
  ...
  const handleSave = async () => {
    ...
    const nameToSave = missionName || `Route ${savedMissions.length + 1}`;
    await saveMissionAction({
      name: nameToSave,
      timestamp: Date.now(),
      path: flightPath,
  ...
  return (
    <div className="absolute top-4 left-4 bottom-4 w-72 bg-slate-900 border border-slate-700/50 rounded-2xl shadow-2xl flex flex-col z-[1000] text-slate-200 overflow-hidden">
                    ...
                    <div key={m.id} className="bg-slate-950 border border-slate-800 rounded-xl p-2.5 flex gap-3 group">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start">
                          ...
                          <button onClick={() => removeMissionAction({ id: m._id })} className="text-slate-700 hover:text-red-500 transition-colors"><Trash2 size={12} /></button>
                        </div>
                        <div className="flex items-center gap-3 mt-1.5">
                          <button onClick={() => loadMissionData(m)} className="text-[9px] text-aviation-orange font-black hover:text-white transition-colors uppercase tracking-widest">Load</button>
      ...
      <div className="px-4 py-3 bg-slate-900 border-t border-slate-800 flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-slate-500">
        <button onClick={() => navigate('/')} className="hover:text-white transition-colors">Home</button>
        <button onClick={() => navigate('/hackathon')} className="hover:text-white transition-colors">Awards</button>
