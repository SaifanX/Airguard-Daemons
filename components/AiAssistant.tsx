...
const AiAssistant: React.FC = () => {
  ...
  const handleSend = async (overrideText?: string) => {
    ...
    const environmentContext = `${weatherContext}\n- Airspace Status: ${zoneContext}`;

    let aiResponseText = "Communication relay weak. Please rephrase your request, Pilot.";
    try {
      const response = await askCaptain({
        userMessage: textToSend,
        riskLevel,
        violations,
        droneModel: `${droneSettings.model} (Operating Height: ${droneSettings.altitude}m)`,
        environmentContext
      });
      if (response) aiResponseText = response;
    } catch (e) {
      console.error(e);
