...
export const getCaptainCritique = async (
  askCaptainAction: (args: any) => Promise<string>,
  userMessage: string,
  riskLevel: number,
  violations: string[],
  flightDetails: any,
  ...
  try {
    const responseText = await askCaptainAction({
      ...
      droneAltitude: flightDetails.altitude || 0,
      weatherContext,
      zoneContext,
    });
    return responseText;
    ...
    return "Relay Error: Could not connect to the AI Tactical Core via secure link. Check logs.";
  }
};
