...
export const getCaptainCritique = async (
  ...
  try {
    const response = await askCaptainAction({
      ...
      riskLevel,
      violations,
      droneModel: flightDetails.model + ` (Operating Height: ${flightDetails.altitude}m)`,
      weatherContext,
      zoneContext
    });

    return response || "Communication relay weak. Please rephrase your request, Pilot.";
  } catch (error: any) {
    console.error("AskCaptain Action Error:", error);
    return "Relay Error: Could not connect to the AI Tactical Core on the backend. Check logs.";
  }
};
